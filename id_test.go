...
var IDs = []IDParts{
	{
		ID{0x00, 0x4d, 0x88, 0xe1, 0x5b, 0x60, 0xf4, 0x86, 0xe4, 0x28, 0x41, 0x2d, 0xc9},
		1300816219,
		[]byte{0x60, 0xf4, 0x86},
		0xe428,
		4271561,
	...
	{
		ID{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
		0,
		[]byte{0x00, 0x00, 0x00},
		0x0000,
		0,
	...
	{
		ID{0x00, 0x00, 0x00, 0x00, 0x00, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0x00, 0x00, 0x01},
		0,
		[]byte{0xaa, 0xbb, 0xcc},
		0xddee,
		1,
...
func TestIDString(t *testing.T) {
	id := ID{0x00, 0x4d, 0x88, 0xe1, 0x5b, 0x60, 0xf4, 0x86, 0xe4, 0x28, 0x41, 0x2d, 0xc9}
	if got, want := id.String(), "016ohoarc3q8dp1884msi"; got != want {
...
func TestIDEncode(t *testing.T) {
	...
	if got, want := string(id.Encode(text)), "016ohoarc3q8dp1884msi"; got != want {
		t.Errorf("Encode() = %v, want %v", got, want)
	}
}

func TestFromString(t *testing.T) {
	got, err := FromString("016ohoarc3q8dp1884msi")
	if err != nil {
		t.Fatal(err)
	}
	want := ID{0x00, 0x4d, 0x88, 0xe1, 0x5b, 0x60, 0xf4, 0x86, 0xe4, 0x28, 0x41, 0x2d, 0xc9}
	if got != want {
...
func TestFromStringInvalid(t *testing.T) {
	...
	id, err := FromString("016ohoarc3q8dp1884ms1")
	if id != nilID {
...
func TestIDJSONMarshaling(t *testing.T) {
	...
	if got, want := string(data), `{"ID":"016ohoarc3q8dp1884msi","Str":"test"}`; got != want {
		t.Errorf("json.Marshal() = %v, want %v", got, want)
	}
}

func TestIDJSONUnmarshaling(t *testing.T) {
	data := []byte(`{"ID":"016ohoarc3q8dp1884msi","Str":"test"}`)
	v := jsonType{}
...
func TestIDJSONUnmarshalingError(t *testing.T) {
	v := jsonType{}
	err := json.Unmarshal([]byte(`{"ID":"016OHOARC3Q8DP1884MSI"}`), &v)
	if err != ErrInvalidID {
...
func TestIDDriverValue(t *testing.T) {
	...
	if want := "016ohoarc3q8dp1884msi"; got != want {
...
func TestIDDriverScan(t *testing.T) {
	got := ID{}
	err := got.Scan("016ohoarc3q8dp1884msi")
	if err != nil {
		t.Fatal(err)
	}
	want := ID{0x00, 0x4d, 0x88, 0xe1, 0x5b, 0x60, 0xf4, 0x86, 0xe4, 0x28, 0x41, 0x2d, 0xc9}
...
func BenchmarkFromString(b *testing.B) {
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			_, _ = FromString("016ohoarc3q8dp1884msi")
		}
	})
}
